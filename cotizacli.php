...
*/
 ...
 if (!$pagina){
 	...
 	$sqla = "select count(*) as canti from cotizacliente INNER JOIN elementoscot ON cotizacliente.cod_ele_cot = elementoscot.cod_ele_cot where  ".$criterio." ";
 	//echo $sqla;// exit();
    $dba  = conecto();
  ...
  // realizo la consulta....
  $criterio.=" order by fe_cot_cli DESC LIMIT ".$TAMANO_PAGINA." OFFSET ".$inicio." ";
  $sqlc = "SELECT * FROM cotizacliente 
			INNER JOIN elementoscot ON cotizacliente.cod_ele_cot = elementoscot.cod_ele_cot
			  where ".$criterio."";
...
<body>
	...
	<p>USUARIO: <?php echo $Xnombre; ?></p>
		...
		<tr>
			...
			<td><div style="background-color:<?php echo $color;?>"><?php echo $fe_cot_cli = date("d-m-Y", strtotime($arr['fe_cot_cli'])); ?></div></td>
            <td><a href="<?php 	$cod_cot_cli = trim($arr['cod_cot_cli']);
								echo 'solicotizacli.php?cod='.$cod_cot_cli; ?>"><button class="button"><?php 
								echo 'Ver';
							?></button></a></td>
		</tr>
        <?php 
