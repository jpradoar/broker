...
if($Xbuscar=="E"){
	...
	if(strlen($nom)!=0)
		$busqueda.= " and cotizachina.cod_ele_cot = '".$nom."' ";

	if(strlen($codba)!=0)
		$busqueda.= " and cod_ele_ba = '".$codba."' ";

	if($feson=="S"){
		$busqueda.= " and fe_cot BETWEEN '".$fecha."' AND  '".$fechaf."' ";
	}	
		

	$sqla = "select count(*) as canti from cotizachina INNER JOIN elementoscot ON cotizachina.cod_ele_cot = elementoscot.cod_ele_cot where ".$busqueda." ";		
   # echo $sqla; exit();
	$dba  = conecto();
	...
	if($cantidad>0){//Si encontro algo		
		$sqlc = " select * from cotizachina INNER JOIN elementoscot ON cotizachina.cod_ele_cot = elementoscot.cod_ele_cot where ".$busqueda." order by nom_ele_cot DESC ";	
		//echo $sqlc;
		$dbc  = conecto();
...
if($_POST){//SEGUNDOS POST
	if($_POST['BTNBusco']){
		header("Location: buscocotiza.php"); 
		exit;	
	}
...
<body>
    ...
    <table class="sombra">
		...
		<tr>
			...
			<td><?php echo $fe_cot   = ordenofecha(trim($arr['fe_cot']));?></td>
            <td>
            <a href="<?php $cod_prod_cotch  = trim($arr['cod_prod_cotch']);
			echo "solicotiza.php?cod=".$cod_prod_cotch; ?>"><input name="BTNir" type="button" class="button" id="BTNir" value=" VER "></a></td>
		</tr>
        <?php 
			}
		}else{
